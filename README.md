# The Bank3 System
## Overview
The repository provides implementation of the basic version of the ``Bank3`` protocols for Wallets and DAOs described in this [note](https://hackmd.io/yR2x-LH_TTWZFso5LJreqw).  This repo only covers a subset of the Bank3 functionalities and does focus on Ethereum. See the above note and [this paper](https://drive.google.com/file/d/13R03H8zSlOdK5KKY2f2qO54yJ6jMHttE/view?usp=sharing) for variants that offers more privacy and flexibility.
 ...
 * A ``TC`` deposit+withdrawal can consume more than 1.3/1.4 milions of GAS whereas in our current ``Bank3`` implementation a deposit (resp. withdrawal) costs about 68k (resp. 35k) and likely we can lower the deposit cost a bit more (e.g., with fixed notes)!
...
Finally, in the folder ``server-side`` launch the command:
```bash
node bank3server.js
```
...
* You can click on "Scan" to get a list of deposits that can be claimed by you, and by just clicking on the "identifier" field you can copy it in the "Withdraw" box.
